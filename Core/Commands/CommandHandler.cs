...
namespace SecretBot123.Core.Commands
{
    class CommandHandler
    {
        ...
        public async Task HandleCommand(SocketMessage s)
        {
            ...
            //argPos decides where the prefix is to be found, 0 = first letter. Change prefix to whatever you'd want if ?? not to your taste
            int argPos = 0;
            string prefix = "??";
                ...
                if(!result.IsSuccess)
                {
                    switch (result.ToString())
                    {
                        default:
                            ...
                            //catches if more than one word is used in the command
                            await s.Channel.SendMessageAsync($"An error occurred! Details: ```" + result.ToString() + "```");
                            break;

                            //catches if an unkown command is used
                        case "UnkownCommand: Unkown command.":
